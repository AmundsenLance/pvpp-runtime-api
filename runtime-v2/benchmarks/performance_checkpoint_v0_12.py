"""PV-PP Runtime v0.12 scheduler optimization checkpoint.
...
if __name__=='__main__':
    ...
    print('feasible scheduler boundary (one shared slot):')
    for n in (5,10,20,50,100): timed_feas(n,n*3)
    print('infeasible one-period-short, witness diagnostics:')
    for n in (5,10,20,30): timed_feas(n,n*3-1)
    print('high-capacity parallel feasible cases:')
    for n in (10,20,50,100):
        rt,cal=synthetic(n,3,cap=n)
        t=time.perf_counter(); rep=rt.assess_recovery_feasibility(cal); dt=time.perf_counter()-t
        print(f"corridors={n} periods=3 cap={n} feasible={rep.jointly_feasible}: {dt*1000:.3f} ms")
