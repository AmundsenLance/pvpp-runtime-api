...
class PVPPRuntime:
    def __init__(self, registry: PVPPRegistry, world: WorldAdapter, projection_service: PolicyProjectionService | None = None, layer1_transition_service: Layer1TransitionService | None = None, memory_retrieval_service: MemoryRetrievalService | None = None, epistemic_update_service: EpistemicUpdateService | None = None, capacity_authority_service: CapacityAuthorityService | None = None, prediction_error_service: PredictionErrorService | None = None, transfer_history_service: TransferHistoryService | None = None):
        self.registry=registry; self.world=world; self.projection_service=projection_service; self.layer1_transition_service=layer1_transition_service; self.memory_retrieval_service=memory_retrieval_service; self.epistemic_update_service=epistemic_update_service; self.capacity_authority_service=capacity_authority_service; self.prediction_error_service=prediction_error_service; self.transfer_history_service=transfer_history_service; self.active_corridors={}
        self._native_execution_authorizations={}
        self._native_execution_authorization_execution_ids={}
        self._native_execution_authorization_sequence=0
        self._native_execution_authorization_lock=threading.Lock()
        # v0.131: runtime-private canonical execution lineage. Public dataclass
        # equality is descriptive only; object identity in these ledgers is authority-bearing.
        self._canonical_execution_licenses={}
        self._canonical_execution_episodes={}
    ...
    def validate_transfer_history_event(self, event: TransferHistoryEvent, *, expected_actor_id: str | None = None):
        violations=[]
        if not isinstance(event, TransferHistoryEvent):
            raise ValueError("transfer-history event must be TransferHistoryEvent")
        if not event.event_id:
            violations.append("transfer-history event_id required")
        if not event.actor_id:
            violations.append("transfer-history actor_id required")
        if expected_actor_id is not None and event.actor_id != expected_actor_id:
            violations.append("transfer-history actor_id mismatch")
        if event.sequence_index is not None and (not isinstance(event.sequence_index,int) or event.sequence_index < 0):
            violations.append("transfer-history sequence_index must be a nonnegative integer when supplied")
        if event.temporal_provenance is not None and not isinstance(event.temporal_provenance, InformationTemporalProvenance):
            violations.append("transfer-history temporal_provenance must be InformationTemporalProvenance")
        return tuple(violations)

    def validate_transfer_history_package(self, package: TransferHistoryPackage, *, expected_actor_id: str | None = None, prior_history: TransferHistoryReference | None = None) -> TransferHistoryValidationAssessment:
        if not isinstance(package, TransferHistoryPackage):
            raise ValueError("transfer-history service must return TransferHistoryPackage")
        violations=[]
        if expected_actor_id is not None and package.actor_id != expected_actor_id:
            violations.append("transfer-history package actor_id mismatch")
        if not isinstance(package.next_history, TransferHistoryReference):
            violations.append("transfer-history package requires next_history reference")
            next_id=None
        else:
            next_id=package.next_history.transfer_history_id
            if package.next_history.actor_id != package.actor_id:
                violations.append("next transfer-history actor_id mismatch")
            if not package.next_history.transfer_history_id:
                violations.append("next transfer_history_id required")
            try:
                nt=float(package.next_history.time)
                if not math.isfinite(nt): violations.append("next transfer-history time must be finite")
            except Exception:
                nt=None; violations.append("next transfer-history time must be numeric")
        if prior_history is not None:
            if package.prior_history != prior_history:
                violations.append("transfer-history package prior_history does not match supplied prior history")
            if prior_history.actor_id != package.actor_id:
                violations.append("prior transfer-history actor_id mismatch")
            try:
                pt=float(prior_history.time)
                if nt is not None and nt < pt-1e-12:
                    violations.append("transfer-history time may not move backward")
            except Exception:
                violations.append("prior transfer-history time must be numeric")
        ids=[]; seq=[]
        for event in package.events:
            violations.extend(self.validate_transfer_history_event(event, expected_actor_id=package.actor_id))
            if isinstance(event,TransferHistoryEvent):
                ids.append(event.event_id)
                if event.sequence_index is not None: seq.append(event.sequence_index)
        if len(ids) != len(set(ids)):
            violations.append("transfer-history event_ids must be unique within package")
        if seq and seq != sorted(seq):
            violations.append("transfer-history events must preserve supplied sequence order")
        return TransferHistoryValidationAssessment(
            valid=not violations, actor_id=package.actor_id, transfer_history_id=next_id,
            event_ids=tuple(ids), violations=tuple(violations),
            notes=("event history is distinct from retained memory and expectation",
                   "event existence does not create current PP or current transition-relevant state",
                   "transfer history supplies evidence but does not itself authorize execution")
        )

    def record_transfer_history(self, prior_history: TransferHistoryReference | None, events: tuple[TransferHistoryEvent, ...]) -> tuple[TransferHistoryPackage, TransferHistoryValidationAssessment]:
        if self.transfer_history_service is None:
            raise RuntimeError("transfer_history_service required")
        if not events:
            raise ValueError("at least one transfer-history event required")
        actor_id=events[0].actor_id
        for event in events:
            ev=self.validate_transfer_history_event(event, expected_actor_id=actor_id)
            if ev: raise ValueError(f"invalid transfer-history event: {ev}")
        if prior_history is not None and prior_history.actor_id != actor_id:
            raise ValueError("prior transfer-history actor_id mismatch")
        package=self.transfer_history_service.record(prior_history,events)
        assessment=self.validate_transfer_history_package(package,expected_actor_id=actor_id,prior_history=prior_history)
        if not assessment.valid:
            raise ValueError(f"invalid transfer-history package: {assessment.violations}")
    ...
    def validate_information_governance_metadata(self, governance: InformationGovernanceMetadata):
        """Validate V2.1 retrieval governance without converting quality into authority.

        A material non-current disposition is preserved as such.  Validation
        does not silently promote it to current positive evidence.
        """
        if not isinstance(governance, InformationGovernanceMetadata):
            raise ValueError("governance must be InformationGovernanceMetadata")
        violations=[]
        disposition=(governance.disposition or "").strip().lower().replace("-","_").replace(" ","_")
        allowed={"current","rejected","superseded","defective","prohibited_for_use",
                 "historically_valid_noncurrent","unresolved","contradicted"}
        if disposition not in allowed:
            violations.append("unrecognized information disposition")
        for surface in governance.authorized_surfaces:
            if not isinstance(surface,str) or not surface.strip():
                violations.append("authorized_surfaces must contain non-empty strings")
                break
        tp=governance.temporal_provenance
        if tp is not None and not isinstance(tp, InformationTemporalProvenance):
            violations.append("temporal_provenance must be InformationTemporalProvenance")
        usable=(not violations and disposition == "current")
        return InformationGovernanceAssessment(
            valid=not violations, disposition=disposition,
            usable_as_current_positive_evidence=usable,
            violations=tuple(violations),
            notes=("quality/confidence does not confer source authority",
                   "relevance does not confer applicability or permitted use",
                   "non-current material disposition is preserved and not promoted to ordinary current positive evidence")
        )
    ...
    def validate_memory_retrieval_package(self, request: MemoryRetrievalRequest,
        ...
        if package.governance is not None:
            try:
                governance_assessment=self.validate_information_governance_metadata(package.governance)
                violations.extend(governance_assessment.violations)
    ...
    def validate_objective_set(
            self, objectives: ObjectiveSet | None,
            actual_state: ActualPersistentStateEnvelope
            ) -> ObjectiveSetValidationAssessment:
        """Validate the V2.1 O_i(t) carrier without interpreting objective content.

        The runtime owns shape, identity, lifecycle and temporal consistency only.
        It does not endorse objectives, rank them, infer motivation, or turn them
        into viability/authority/selection semantics.
        """
        if objectives is None:
            return ObjectiveSetValidationAssessment(
                True, actual_state.actor_id, "", (),
                ("objective-free viability analysis remains valid",)
            )
        if not isinstance(objectives, ObjectiveSet):
            raise ValueError("objectives must be ObjectiveSet or None")
        violations=[]
        if not str(objectives.actor_id).strip():
            violations.append("objective set requires actor_id")
        elif objectives.actor_id != actual_state.actor_id:
            violations.append("objective set actor_id mismatch")
        try:
            ot=float(objectives.time)
            if not math.isfinite(ot):
                violations.append("objective set time must be finite")
            elif not math.isclose(ot,float(actual_state.time),rel_tol=0.0,abs_tol=1e-12):
                violations.append("objective set time must align with current decision-cycle time")
        except Exception:
            violations.append("objective set time must be numeric")
        if not str(objectives.objective_set_id).strip():
            violations.append("objective set requires stable identity")
        if not str(objectives.version).strip():
            violations.append("objective set requires version identity")
        seen=set()
        allowed_status={"active","suspended","completed","abandoned","expired"}
        allowed_relations={"dependency","conflict","prerequisite","complementarity"}
        for obj in objectives.objectives:
            if not isinstance(obj,QualifiedObjectiveRecord):
                violations.append("objective set entries must be QualifiedObjectiveRecord")
                continue
            oid=str(obj.id).strip()
            if not oid:
                violations.append("objective requires stable id")
            elif oid in seen:
                violations.append(f"duplicate objective id: {oid}")
            seen.add(oid)
            if obj.content is None or (isinstance(obj.content,str) and not obj.content.strip()):
                violations.append(f"objective {oid or '<unknown>'} requires content")
            if not str(obj.source).strip():
                violations.append(f"objective {oid or '<unknown>'} requires source")
            if obj.scope is None or (isinstance(obj.scope,str) and not obj.scope.strip()):
                violations.append(f"objective {oid or '<unknown>'} requires scope")
            if obj.status not in allowed_status:
                violations.append(f"objective {oid or '<unknown>'} has invalid lifecycle status")
            elif obj.status != "active":
                violations.append(f"objective {oid or '<unknown>'} is {obj.status} and cannot remain in active O_i(t)")
            if obj.authority is None:
                violations.append(f"objective {oid or '<unknown>'} requires authority/provenance")
            if not str(obj.version).strip():
                violations.append(f"objective {oid or '<unknown>'} requires version identity")
            tb=obj.temporal_bounds
            if tb is not None:
                if not isinstance(tb,ObjectiveTemporalBounds):
                    violations.append(f"objective {oid or '<unknown>'} temporal_bounds must be ObjectiveTemporalBounds")
                else:
                    for name in ("start","deadline","horizon","expiration"):
                        value=getattr(tb,name)
                        if value is not None:
                            try:
                                if not math.isfinite(float(value)):
                                    violations.append(f"objective {oid or '<unknown>'} {name} must be finite")
                            except Exception:
                                violations.append(f"objective {oid or '<unknown>'} {name} must be numeric")
                    if tb.start is not None and float(actual_state.time) < float(tb.start):
                        violations.append(f"objective {oid or '<unknown>'} is not yet temporally applicable")
                    if tb.expiration is not None and float(actual_state.time) >= float(tb.expiration):
                        violations.append(f"objective {oid or '<unknown>'} is expired at current decision time")
            for rel in obj.relations:
                if not str(rel.related_objective_id).strip():
                    violations.append(f"objective {oid or '<unknown>'} relation requires related objective id")
                if not str(rel.relation).strip():
                    violations.append(f"objective {oid or '<unknown>'} relation requires relation label")
                # V2.1 permits 'other declared relation'; known labels are not exhaustive.
        return ObjectiveSetValidationAssessment(
            not violations, objectives.actor_id, objectives.objective_set_id, tuple(violations),
            (
                "O_i(t) is adjacent to P_i(t), not a component of P_i(t) or Layer-1 state",
                "objective representation is not endorsement or execution authority",
                "objective records do not rank policies, redefine viability, or alter canonical operator order",
                "conflicting active objectives may coexist without runtime-invented ordering",
            )
        )
    ...
    def validate_objective_discovery_hints(self, objectives: ObjectiveSet | None, hints=()) -> ObjectiveDiscoveryAssessment:
        """Validate bounded objective relevance without changing Graph admission.

        Only already-registered Graph identifiers may be referenced. Retrieval
        cues are carried as application data and confer no source authority or
        permitted-use status.  The assessment is informational only.
        """
        hints=tuple(hints or ())
        if objectives is None:
            if hints:
                return ObjectiveDiscoveryAssessment(False, violations=("objective discovery hints require active O_i(t)",))
            return ObjectiveDiscoveryAssessment(True, notes=("no active objective set; no objective-directed discovery",))
        active={o.id for o in objectives.objectives}
        violations=[]; tids=[]; iids=[]; cues=[]
        for h in hints:
            if not isinstance(h,ObjectiveDiscoveryHint):
                violations.append("objective discovery entries must be ObjectiveDiscoveryHint")
                continue
            if h.objective_id not in active:
                violations.append(f"objective discovery hint references inactive or absent objective: {h.objective_id}")
            for tid in h.graph_transformation_ids:
                if tid not in self.registry.graph_transformations:
                    violations.append(f"objective discovery hint references unregistered graph transformation: {tid}")
                else: tids.append(tid)
            for iid in h.graph_instance_ids:
                if iid not in self.registry.graph_instances:
                    violations.append(f"objective discovery hint references unregistered graph instance: {iid}")
                else: iids.append(iid)
            cues.extend(h.retrieval_cues)
        return ObjectiveDiscoveryAssessment(
            not violations,objectives.objective_set_id,len(hints),tuple(dict.fromkeys(tids)),
            tuple(dict.fromkeys(iids)),tuple(cues),tuple(violations),(
                "objective relevance is bounded to already registered structural references and application retrieval cues",
                "objective relevance does not admit Graph structure, establish reachability, rank policies, or confer information-use authority",
            )
        )

    def evaluate_objective_responsiveness(self, state, policy_space, objectives: ObjectiveSet | None, adapter=None) -> ObjectiveResponsivenessAssessment:
        """Evaluate application-owned Resp(pi,o) without consuming it as a gate.

        Every candidate/objective pair is represented when an adapter is supplied.
        The result is descriptive only and is never fed back into Pi, Constraints,
        Adequacy, Sigma, epsilon, or execution authorization.
        """
        if objectives is None or not objectives.objectives:
            return ObjectiveResponsivenessAssessment(True,notes=("no active objectives; responsiveness not required",))
        if adapter is None:
            return ObjectiveResponsivenessAssessment(True,notes=("no responsiveness adapter supplied; objective-policy relations remain unresolved by the runtime",))
        allowed={"supports","neutral","conflicts","unresolved"}
        records=[]; violations=[]
        for candidate in policy_space.candidates:
            for objective in objectives.objectives:
                rec=adapter.objective_responsiveness(state,candidate.id,tuple(candidate.action_ids),objective)
                if not isinstance(rec,ObjectiveResponsivenessRecord):
                    violations.append(f"Resp({candidate.id},{objective.id}) did not return ObjectiveResponsivenessRecord")
                    continue
                if rec.policy_id != candidate.id or rec.objective_id != objective.id:
                    violations.append(f"Resp({candidate.id},{objective.id}) identity mismatch")
                if rec.relation not in allowed:
                    violations.append(f"Resp({candidate.id},{objective.id}) invalid relation: {rec.relation}")
                records.append(rec)
        return ObjectiveResponsivenessAssessment(
            not violations,tuple(records),tuple(violations),(
                "Resp(pi,o) is descriptive/informational and is not a qualification gate",
                "responsiveness does not admit, exclude, rank, authorize, prove adequacy, establish viability, or select a policy",
                "indirect and prerequisite policies may remain neutral or unresolved without pruning",
            )
        )

    def prepare_canonical_cycle_snapshot(self, actual_state: ActualPersistentStateEnvelope, objectives: ObjectiveSet | None = None) -> CanonicalCycleSnapshot:
        ...
        if not pv.valid:
            raise ValueError(f"invalid perceived decision state: {pv.violations}")
        ov=self.validate_objective_set(objectives,actual_state)
        if not ov.valid:
            raise ValueError(f"invalid objective set: {ov.violations}")
    ...
    def evaluate_integrated_canonical_cycle(self, actual_state: ActualPersistentStateEnvelope,
                                            ...
                                            objective_responsiveness_adapter=None,
                                            objective_discovery_hints=(),
                                            execute: bool = False,
        ...
        snapshot=self.prepare_canonical_cycle_snapshot(actual_state,objectives)
        ...
        decision=self.evaluate_canonical_decision_cycle(
            ...
            perceived_decision_state=snapshot.perceived_decision_state, objectives=objectives,
            objective_responsiveness_adapter=objective_responsiveness_adapter,
            objective_discovery_hints=objective_discovery_hints
        )
        # Surface the informational V2.1 objective assessments on the integrated
        # decision result without making either assessment an operator input.
        discovery=self.validate_objective_discovery_hints(objectives,objective_discovery_hints)
        responsiveness=None
        if decision.pi_construction is not None and decision.pi_construction.policy_space is not None:
            responsiveness=self.evaluate_objective_responsiveness(
                snapshot.represented_state,decision.pi_construction.policy_space,objectives,objective_responsiveness_adapter
            )
        decision=replace(decision,objective_discovery=discovery,objective_responsiveness=responsiveness)
        base_notes=(
            "one host-invoked integrated pass; no autonomous runtime loop was created",
            "Layer 1 actual state remained opaque to Layer 2",
            "O_i(t) was carried adjacent to P_i(t) and was not consumed as a canonical decision operator",
        ) + ((
    ...
    def audit_cycle_artifact_integrity(self, cycle: CanonicalDecisionCycleAssessment, *, expected_state_id: str | None = None, expected_state_time: float | None = None) -> CycleArtifactIntegrityAssessment:
        ...
        if graph and set(graph.governing_domain_ids)!=set(governing): violations.append('Graph governing-domain identity does not match G')
        if pi:
            if set(pi.governing_domain_ids)!=set(governing): violations.append('Pi governing-domain identity does not match G')
        ...
        if framing:
            if set(framing.governing_domain_ids)!=set(governing): violations.append('Domain Framing governing identity does not match G')
    ...
    def evaluate_canonical_decision_cycle(self, state, request: CanonicalDecisionCycleRequest, *, perceived_decision_state: PerceivedDecisionState | None = None, objectives: ObjectiveSet | None = None, objective_responsiveness_adapter=None, objective_discovery_hints=()) -> CanonicalDecisionCycleAssessment:
        ...
        objective_discovery=self.validate_objective_discovery_hints(objectives,objective_discovery_hints)
        if not objective_discovery.valid:
            raise ValueError(f"invalid objective discovery hints: {objective_discovery.violations}")
        perceived=self.world.perceive(state)
        ...
        objective_responsiveness=self.evaluate_objective_responsiveness(
            state,pi.policy_space,objectives,objective_responsiveness_adapter
        )
        if not objective_responsiveness.valid:
            raise ValueError(f"invalid objective responsiveness: {objective_responsiveness.violations}")

        pi_before_joint_recovery_binding=pi
        ...
        if joint_recovery.status=="active_capacity_authority_required":
            return CanonicalDecisionCycleAssessment(
                ...
                pi_before_joint_recovery_binding=pi_before_joint_recovery_binding
            )
        ...
        ),joint_recovery,binding,audit,projection_input_license,projection_model_authority, None, pi_before_joint_recovery_binding)
    ...
    def build_execution_license_from_cycle(self, cycle: CanonicalDecisionCycleAssessment,
        ...
        if mode == "terminal_infeasibility":
            ...
            self._canonical_execution_licenses[id(license)]=(license,id(cycle),"issued")
            return license
    ...
    def _validate_native_license_posture(self, license: ExecutionLicenseEnvelope) -> None:
        if not license.entry_authorized:
            raise ValueError("native execution authorization requires authorized execution license")
        if license.selection_mode == "standard":
            ok=(license.pi_complete and license.constraints_feasible and license.framing_valid and license.adequacy_sufficient)
        elif license.selection_mode == "recovery_unavailable_fallback":
            ok=(license.pi_complete and license.constraints_feasible and license.framing_valid and not license.adequacy_sufficient)
        ...
        if not ok:
            raise ValueError("native execution authorization rejected inconsistent license posture")

    def issue_native_execution_authorization(self, episode: ExecutionEpisode, action_id: str, binding_registry, *, decision_cycle_id: str, configuration_id: str | None = None, attempt: int = 1) -> NativeExecutionAuthorization:
        if not isinstance(episode, ExecutionEpisode): raise TypeError("execution episode required")
        ep_lineage=self._canonical_execution_episodes.get(episode.episode_id)
        if ep_lineage is None or ep_lineage[0] is not episode or ep_lineage[2] != "current":
            raise ValueError("native execution authorization requires runtime-issued canonical execution episode")
        lic_lineage=self._canonical_execution_licenses.get(id(episode.license))
        if lic_lineage is None or lic_lineage[0] is not episode.license or lic_lineage[2] != "instantiated" or ep_lineage[1] != id(episode.license):
            raise ValueError("native execution authorization requires current canonical execution license lineage")
        if not episode.active or not episode.entry_sufficient: raise ValueError("native execution authorization requires active sufficient episode")
        self._validate_native_license_posture(episode.license)
        if action_id not in episode.license.action_ids: raise ValueError("native execution authorization action is not licensed")
        identity=binding_registry.identity_for_action(action_id)
        if identity is None: raise KeyError(f"no execution binding registered for action: {action_id}")
        if not decision_cycle_id.strip(): raise ValueError("decision_cycle_id required for native execution authorization")
        if attempt < 1: raise ValueError("native execution authorization attempt must be >= 1")
        with self._native_execution_authorization_lock:
            self._native_execution_authorization_sequence += 1
            seq=self._native_execution_authorization_sequence
            auth=NativeExecutionAuthorization(f"nauth-{uuid4()}",episode.episode_id,decision_cycle_id,episode.license.selected_policy_id,action_id,identity.binding_id,identity.implementation_version,configuration_id,attempt,seq,"issued",("runtime-private ledger state controls authority","single-use consume-before-call capability"))
            self._native_execution_authorizations[auth.authorization_id]=auth
            self._native_execution_authorization_execution_ids[auth.authorization_id]=f"exec-{uuid4()}"
        return auth

    def native_execution_authorization_status(self, authorization_id: str) -> str | None:
        rec=self._native_execution_authorizations.get(authorization_id); return None if rec is None else rec.status

    def native_execution_authorizations(self, *, episode_id: str | None = None, decision_cycle_id: str | None = None) -> tuple[NativeExecutionAuthorization, ...]:
        records=tuple(self._native_execution_authorizations.values())
        if episode_id is not None: records=tuple(x for x in records if x.episode_id==episode_id)
        if decision_cycle_id is not None: records=tuple(x for x in records if x.decision_cycle_id==decision_cycle_id)
        return tuple(replace(x) for x in sorted(records,key=lambda x:x.issued_sequence))

    def invalidate_native_execution_authorizations(self, *, episode_id: str | None = None, decision_cycle_id: str | None = None, action_id: str | None = None, reason: str) -> tuple[str, ...]:
        if episode_id is None and decision_cycle_id is None and action_id is None: raise ValueError("native execution authorization invalidation requires scope identity")
        if not reason.strip(): raise ValueError("native execution authorization invalidation reason required")
        changed=[]
        with self._native_execution_authorization_lock:
            for aid,rec in tuple(self._native_execution_authorizations.items()):
                if rec.status != "issued": continue
                if episode_id is not None and rec.episode_id != episode_id: continue
                if decision_cycle_id is not None and rec.decision_cycle_id != decision_cycle_id: continue
                if action_id is not None and rec.action_id != action_id: continue
                self._native_execution_authorizations[aid]=replace(rec,status="invalidated",notes=rec.notes+(f"invalidated: {reason}",)); changed.append(aid)
            # The same explicit stale-lineage signal also retires matching active
            # episode capabilities. The native layer still does not infer a re-entry stage.
            episode_ids=set()
            if episode_id is not None: episode_ids.add(episode_id)
            if decision_cycle_id is not None:
                episode_ids.update(rec.episode_id for rec in self._native_execution_authorizations.values() if rec.decision_cycle_id==decision_cycle_id)
            for eid in episode_ids:
                ep=self._canonical_execution_episodes.get(eid)
                if ep is not None and ep[2] == "current": self._canonical_execution_episodes[eid]=(ep[0],ep[1],"invalidated")
        return tuple(changed)

    def native_execution_authorization_execution_id(self, authorization_id: str) -> str | None:
        return self._native_execution_authorization_execution_ids.get(authorization_id)

    def invoke_authorized_native(self, authorization: NativeExecutionAuthorization, binding_registry, *args, configuration_id: str | None = None, **kwargs):
        if not isinstance(authorization, NativeExecutionAuthorization): raise TypeError("native execution authorization required")
        with self._native_execution_authorization_lock:
            ledger=self._native_execution_authorizations.get(authorization.authorization_id)
            if ledger is None: raise KeyError("unknown native execution authorization")
            if ledger.status != "issued": raise RuntimeError(f"native execution authorization lifecycle status is {ledger.status}")
            if authorization is not ledger: raise ValueError("native execution authorization object is not the runtime-issued ledger record")
            if authorization.attempt < 1: raise ValueError("native execution authorization attempt mismatch")
            if authorization.configuration_id != configuration_id: raise ValueError("native execution authorization configuration mismatch")
            identity=binding_registry.identity_for_action(authorization.action_id)
            if identity is None: raise KeyError(f"execution binding not registered for action: {authorization.action_id}")
            if identity.binding_id != authorization.binding_id: raise ValueError("native execution authorization binding mismatch")
            if identity.implementation_version != authorization.binding_implementation_version: raise ValueError("native execution authorization binding implementation version mismatch")
            execution_id=self._native_execution_authorization_execution_ids[authorization.authorization_id]
            control=binding_registry.control_state(execution_id)
            if control.state in {"paused","aborted","reauthorization_required"}:
                self._native_execution_authorizations[authorization.authorization_id]=replace(ledger,status="invalidated",notes=ledger.notes+(f"invalidated by native control state: {control.state}",))
                raise RuntimeError(f"native execution authorization invalidated by control state {control.state}")
            self._native_execution_authorizations[authorization.authorization_id]=replace(ledger,status="consumed",notes=ledger.notes+("consumed immediately before callable entry",))
        context=ExecutionContext(execution_id,authorization.action_id,authorization.binding_id,authorization.decision_cycle_id,authorization.attempt,None,authorization.configuration_id,{})
        return binding_registry._invoke_authorized_context(context,*args,**kwargs)
    ...
    def instantiate_execution(self, episode_id: str, license: ExecutionLicenseEnvelope, *,
        ...
        license_lineage=self._canonical_execution_licenses.get(id(license))
        if license_lineage is None or license_lineage[0] is not license or license_lineage[2] != "issued":
            raise ValueError("epsilon execution requires runtime-recognized current canonical execution license")
        self._canonical_execution_licenses[id(license)]=(license,license_lineage[1],"instantiated")
        ...
        self._canonical_execution_episodes[episode_id]=(episode,id(license),"current" if episode.active else "inactive")
        if not licensed:
    ...
    def advance_execution(self, episode: ExecutionEpisode,
        ...
        lineage=self._canonical_execution_episodes.get(episode.episode_id)
        if lineage is not None and lineage[1] == id(episode.license) and lineage[2] == "current":
            self._canonical_execution_episodes[episode.episode_id]=(next_episode,id(episode.license),"terminal" if terminal else "current")
            if terminal:
                self.invalidate_native_execution_authorizations(episode_id=episode.episode_id,reason="execution episode became terminal")
        if terminal:
    ...
    def validate_access_operability_state(self, state: AccessOperabilityState) -> AccessOperabilityValidationAssessment:
        """Validate representation of V2 access/control/operability dimensions only.

        No dimension is inferred from another and this assessment does not decide
        PP, feasibility, adequacy, authorization, or selection.
        """
        violations=[]
        if not isinstance(state,AccessOperabilityState):
            raise ValueError("access/operability adapter must return AccessOperabilityState")
        if not state.resource_id:
            violations.append("resource_id is required")
        try:
            t=float(state.observed_time)
            if not math.isfinite(t):
                violations.append("observed_time must be finite")
        except Exception:
            violations.append("observed_time must be numeric")
        for name in ("possessed","available","permitted","controlled","operable"):
            value=getattr(state,name)
            if value is not None and not isinstance(value,bool):
                violations.append(f"{name} must be bool or None")
        if any(not str(x) for x in state.provenance_ids):
            violations.append("provenance_ids may not contain empty identities")
        return AccessOperabilityValidationAssessment(
            not violations,state.resource_id,tuple(violations),(
                "access dimensions are independent represented facts",
                "validation does not infer PP, feasibility, adequacy, authorization, or selection",
            )
        )

    def validate_productive_configuration_state(self, state: ProductiveConfigurationState) -> ProductiveConfigurationValidationAssessment:
        """Validate a non-scalar configuration/joint-feasibility carrier."""
        violations=[]
        if not isinstance(state,ProductiveConfigurationState):
            raise ValueError("configuration adapter must return ProductiveConfigurationState")
        if not state.configuration_id:
            violations.append("configuration_id is required")
        if not state.component_ids:
            violations.append("at least one component_id is required")
        if len(set(state.component_ids)) != len(state.component_ids):
            violations.append("component_ids must be unique")
        if len(set(state.dependency_ids)) != len(state.dependency_ids):
            violations.append("dependency_ids must be unique")
        if any(not str(x) for x in (*state.component_ids,*state.dependency_ids,*state.evidence_ids)):
            violations.append("component, dependency, and evidence identities may not be empty")
        if state.jointly_feasible is not None and not isinstance(state.jointly_feasible,bool):
            violations.append("jointly_feasible must be bool or None")
        return ProductiveConfigurationValidationAssessment(
            not violations,state.configuration_id,tuple(violations),(
                "joint feasibility is a represented domain fact, not a scalar PP aggregation",
                "configuration representation does not authorize an action or select a policy",
            )
        )

    def validate_recovery_resource_state(self, state: RecoveryResourceState) -> RecoveryResourceValidationAssessment:
        """Validate representation without inferring recovery from reserve or capability."""
        violations=[]
        if not state.recovery_id.strip():
            violations.append("recovery_id must be non-empty")
        if state.observed_time < 0:
            violations.append("observed_time must be nonnegative")
        if len(set(state.prerequisite_ids)) != len(state.prerequisite_ids):
            violations.append("prerequisite_ids must be unique")
        if any(not x.strip() for x in state.prerequisite_ids):
            violations.append("prerequisite_ids must be non-empty")
        if len(set(state.evidence_ids)) != len(state.evidence_ids):
            violations.append("evidence_ids must be unique")
        return RecoveryResourceValidationAssessment(
            valid=not violations, recovery_id=state.recovery_id, violations=tuple(violations),
            notes=("reserve, recovery capability, and current reachability remain independent represented facts",
                   "validation does not establish adequacy, authorization, or selection"))

    def validate_qualified_unit_state(self, state: QualifiedUnitState) -> QualifiedUnitValidationAssessment:
        """Validate neutral unit/nesting metadata without conferring qualification."""
        violations=[]
        if not state.unit_id.strip():
            violations.append("unit_id must be non-empty")
        if not state.unit_type.strip():
            violations.append("unit_type must be non-empty")
        if len(set(state.member_ids)) != len(state.member_ids):
            violations.append("member_ids must be unique")
        if state.unit_id in state.member_ids:
            violations.append("unit may not list itself as a member")
        if state.parent_unit_id == state.unit_id:
            violations.append("unit may not be its own parent")
        if state.productive_role_qualified and not state.qualification_evidence_ids:
            violations.append("productive-role qualification requires explicit evidence")
        if len(set(state.qualification_evidence_ids)) != len(state.qualification_evidence_ids):
            violations.append("qualification_evidence_ids must be unique")
        return QualifiedUnitValidationAssessment(
            valid=not violations, unit_id=state.unit_id, violations=tuple(violations),
            notes=("membership and nesting do not establish collective productive qualification",))

    def validate_transition_relevant_state_basis(
            self, state: ActualPersistentStateEnvelope, basis: TransitionRelevantStateBasis
    ) -> TransitionRelevantStateSufficiencyAssessment:
        """Validate the V2.1 prospective state-sufficiency declaration.

        Semantic completeness remains host/application owned.  The runtime
        validates identity, declared context, evidence/reference presence and
        the host's explicit completeness assertion only.
        """
        violations=[]
        if not basis.basis_id:
            violations.append("basis_id is required")
        if basis.actor_id != state.actor_id:
            violations.append("state-sufficiency actor_id does not match actual state")
        if basis.state_id != state.state_id:
            violations.append("state-sufficiency state_id does not match actual state")
        if not basis.modeled_purpose:
            violations.append("modeled_purpose is required")
        if not basis.admissible_context_id:
            violations.append("admissible_context_id is required")
        if len(set(basis.material_state_surface_ids)) != len(basis.material_state_surface_ids):
            violations.append("material_state_surface_ids must be unique")
        if len(set(basis.authoritative_reference_ids)) != len(basis.authoritative_reference_ids):
            violations.append("authoritative_reference_ids must be unique")
        if not (basis.material_state_surface_ids or basis.authoritative_reference_ids):
            violations.append("state sufficiency requires represented state surfaces or authoritative references")
        if not basis.host_validated_complete:
            violations.append("host/application has not declared transition-relevant state complete")
        return TransitionRelevantStateSufficiencyAssessment(
            sufficient=not violations, basis_id=basis.basis_id, actor_id=basis.actor_id,
            state_id=basis.state_id, violations=tuple(violations),
            notes=("prospective completeness is host/application owned; complete causal history is not required",
                   "historical event evidence is not live state unless its present consequence is represented or referenced",))

    def assess_transition_relevant_state_pair(
            self, left: TransitionRelevantStateBasis, right: TransitionRelevantStateBasis, *,
            left_transition_structure_id: str, right_transition_structure_id: str
    ) -> TransitionRelevantStatePairAssessment:
        """Hostile-test helper for the same-complete-state/context invariant.

        If two declarations claim the same complete represented surfaces/references,
        modeled purpose and admissible context, different modeled transition or
        reachability structure indicates that a causally active consequence was
        omitted from the represented current state.
        """
        violations=[]
        same_basis=(left.modeled_purpose == right.modeled_purpose and
                    left.admissible_context_id == right.admissible_context_id and
                    left.material_state_surface_ids == right.material_state_surface_ids and
                    left.authoritative_reference_ids == right.authoritative_reference_ids and
                    left.host_validated_complete and right.host_validated_complete)
        if same_basis and left_transition_structure_id != right_transition_structure_id:
            violations.append("same complete transition-relevant state and admissible context produced different modeled transition/reachability structure")
        return TransitionRelevantStatePairAssessment(
            consistent=not violations, left_basis_id=left.basis_id, right_basis_id=right.basis_id,
            violations=tuple(violations),
            notes=("historical origin alone does not justify different prospective structure",))
    ...
    def assess_replay_sufficiency(self, package: DecisionReplayPackage) -> ReplaySufficiencyAssessment:
        """Validate the minimum V2.1 historical reconstruction package.

        Sufficiency is evidentiary only. It never makes retained material live
        state and never licenses execution or a later decision.
        """
        if not isinstance(package, DecisionReplayPackage):
            raise ValueError("replay assessment requires DecisionReplayPackage")
        missing=[]; violations=[]
        if not package.decision_episode_id: missing.append("decision_episode_id")
        if not package.actor_id: missing.append("actor_id")
        if package.decision_time is None: missing.append("decision_time")
        if not package.framework_version: missing.append("framework_version")
        if not package.runtime_version: missing.append("runtime_version")
        if package.actual_state is None and package.actual_state_reference is None:
            missing.append("actual_state_or_reference")
        if package.perceived_decision_state is None:
            missing.append("perceived_decision_state")
        if package.decision is None:
            missing.append("governing_decision_pipeline")
        if package.actual_state is not None:
            if package.actual_state.actor_id != package.actor_id:
                violations.append("actual state actor does not match replay actor")
            if package.actual_state.time > package.decision_time:
                violations.append("actual state is later than replay decision time")
        if package.perceived_decision_state is not None:
            if package.perceived_decision_state.actor_id != package.actor_id:
                violations.append("perceived state actor does not match replay actor")
            if package.perceived_decision_state.time > package.decision_time:
                violations.append("perceived state is later than replay decision time")
        if package.objective_set is not None:
            if package.objective_set.actor_id != package.actor_id:
                violations.append("objective set actor does not match replay actor")
            if package.objective_set.time > package.decision_time:
                violations.append("objective set is later than replay decision time")
        if package.retrieval is not None:
            if package.retrieval.actor_id != package.actor_id:
                violations.append("retrieval actor does not match replay actor")
            if package.retrieval.time > package.decision_time:
                violations.append("retrieval is later than replay decision time")
            if package.retrieval.governance is not None:
                ga=self.validate_information_governance_metadata(package.retrieval.governance)
                if not ga.valid:
                    violations.extend(f"retrieval governance: {v}" for v in ga.violations)
        if package.expectation_state is not None and package.expectation_state.actor_id != package.actor_id:
            violations.append("expectation state actor does not match replay actor")
        for ref in package.transfer_history_references:
            if ref.actor_id != package.actor_id:
                violations.append("transfer-history reference actor does not match replay actor")
            if ref.time > package.decision_time:
                violations.append("transfer-history reference is later than replay decision time")
        if package.runtime_configuration is not None:
            if package.runtime_configuration.framework_version != package.framework_version:
                violations.append("runtime configuration framework version mismatch")
            if package.runtime_configuration.runtime_version != package.runtime_version:
                violations.append("runtime configuration runtime version mismatch")
        if package.epsilon_result is not None and package.execution_license is None:
            violations.append("epsilon replay evidence requires its execution license")
        if package.transition_result is not None and package.epsilon_result is None:
            violations.append("Layer-1 transition replay evidence requires epsilon evidence")
        if package.decision is not None and package.decision.stopped_at and package.stop_condition is None:
            # A stopped pipeline can still be replayed, but its stop must be explicit.
            if package.decision.stopped_at != "Sigma" or package.decision.selection is None or package.decision.selection.selected_policy_id is None:
                missing.append("stop_condition")
        return ReplaySufficiencyAssessment(
            not missing and not violations, package.decision_episode_id, tuple(missing), tuple(violations),
            ("replay evidence is historical/audit material and confers no live-state or execution authority",
             "later knowledge must not be substituted for decision-time artifacts")
        )

    def build_decision_replay_package(self, *, decision_episode_id: str, snapshot: CanonicalCycleSnapshot,
                                      decision: CanonicalDecisionCycleAssessment, framework_version: str = "2.1",
                                      runtime_version: str = "0.137", retrieval: MemoryRetrievalPackage | None = None,
                                      expectation_state: ExpectationStateReference | None = None,
                                      transfer_history_references=(), execution_license=None, epsilon_result=None,
                                      transition_result=None, final_actual_state=None, runtime_configuration=None,
                                      source_snapshot_ids=(), governing_rule_ids=(), stop_condition=None,
                                      temporal_provenance=None, metadata=None) -> DecisionReplayPackage:
        """Assemble replay evidence from already-existing decision-time artifacts.

        The method copies/references evidence; it does not re-run the decision,
        reconstruct missing evidence from current memory, or authorize anything.
        """
        if not isinstance(snapshot, CanonicalCycleSnapshot):
            raise ValueError("replay package requires CanonicalCycleSnapshot")
        if not isinstance(decision, CanonicalDecisionCycleAssessment):
            raise ValueError("replay package requires CanonicalDecisionCycleAssessment")
        pkg=DecisionReplayPackage(
            decision_episode_id=decision_episode_id, actor_id=snapshot.actual_state.actor_id,
            decision_time=snapshot.actual_state.time, framework_version=framework_version, runtime_version=runtime_version,
            actual_state=snapshot.actual_state, perceived_decision_state=snapshot.perceived_decision_state,
            objective_set=snapshot.objective_set, retrieval=retrieval, expectation_state=expectation_state,
            transfer_history_references=tuple(transfer_history_references), decision=decision,
            execution_license=execution_license, epsilon_result=epsilon_result, transition_result=transition_result,
            final_actual_state=final_actual_state, runtime_configuration=runtime_configuration,
            source_snapshot_ids=tuple(source_snapshot_ids), governing_rule_ids=tuple(governing_rule_ids),
            stop_condition=stop_condition, temporal_provenance=temporal_provenance, metadata=dict(metadata or {}))
        return pkg

    def validate_runtime_configuration_provenance(self, record: RuntimeConfigurationProvenance) -> RuntimeConfigurationProvenanceAssessment:
        """Validate V2 configuration identity without granting adapter operator authority."""
        violations=[]
        if not record.configuration_id.strip(): violations.append("configuration_id must be nonempty")
        if not record.framework_version.strip(): violations.append("framework_version must be nonempty")
        if not record.runtime_version.strip(): violations.append("runtime_version must be nonempty")
        if record.created_at < 0: violations.append("created_at must be nonnegative")
        roles={"state_measurement","action_structure","projection_world_model","constraint_evidence","execution_transition"}
        ids=[]; seen_roles=[]
        for a in record.adapter_identities:
            if not a.adapter_id.strip(): violations.append("adapter_id must be nonempty")
            if not a.implementation_version.strip(): violations.append(f"adapter {a.adapter_id or '<empty>'} implementation_version must be nonempty")
            if a.adapter_role not in roles: violations.append(f"unsupported adapter_role: {a.adapter_role}")
            ids.append(a.adapter_id); seen_roles.append(a.adapter_role)
        if len(ids) != len(set(ids)): violations.append("adapter identities must be unique")
        if len(seen_roles) != len(set(seen_roles)): violations.append("adapter roles must be unique within one configuration")
        if len(record.source_ids) != len(set(record.source_ids)): violations.append("source_ids must be unique")
        return RuntimeConfigurationProvenanceAssessment(not violations, record.configuration_id, tuple(violations), ("adapter identity is provenance only and does not confer canonical operator authority",))

    def assess_architecture_adaptation_invariance(self, profile, configurations):
        """Check that domain/application adaptation changes adapters, not the canonical operator architecture."""
        from .models import ArchitectureAdaptationAssessment
        violations=[]
        cfgs=tuple(configurations)
        if not profile.profile_id.strip(): violations.append("profile_id must be nonempty")
        if not profile.canonical_stage_ids: violations.append("canonical_stage_ids must be nonempty")
        if len(profile.canonical_stage_ids)!=len(set(profile.canonical_stage_ids)): violations.append("canonical_stage_ids must be unique")
        if len(profile.required_adapter_roles)!=len(set(profile.required_adapter_roles)): violations.append("required_adapter_roles must be unique")
        if len(cfgs)<2: violations.append("at least two runtime configurations are required for cross-domain invariance assessment")
        ids=[]; roles_by={}
        required=set(profile.required_adapter_roles); prohibited=set(profile.prohibited_adapter_roles)
        for rec in cfgs:
            a=self.validate_runtime_configuration_provenance(rec)
            ids.append(rec.configuration_id)
            roles=tuple(x.adapter_role for x in rec.adapter_identities)
            roles_by[rec.configuration_id]=roles
            if not a.valid: violations.extend(f"configuration {rec.configuration_id or '<empty>'}: {v}" for v in a.violations)
            missing=required-set(roles)
            if missing: violations.append(f"configuration {rec.configuration_id}: missing required adapter roles: {','.join(sorted(missing))}")
            bad=prohibited & set(roles)
            if bad: violations.append(f"configuration {rec.configuration_id}: prohibited adapter roles: {','.join(sorted(bad))}")
        if len(ids)!=len(set(ids)): violations.append("configuration identities must be unique")
        return ArchitectureAdaptationAssessment(not violations, profile.profile_id, tuple(ids), tuple(profile.canonical_stage_ids), roles_by, tuple(violations), (
            "adapter diversity is permitted; canonical stage identity remains invariant",
            "this assessment is instrumentation only and confers no canonical operator or execution authority",
        ))
