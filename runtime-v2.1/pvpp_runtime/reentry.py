...
def derive_objective_lifecycle_invalidations(
    dependencies: tuple["GovernanceArtifactDependency", ...],
    change: "ObjectiveLifecycleChange",
) -> "ObjectiveLifecycleInvalidationAssessment":
    """Invalidate only artifacts explicitly dependent on changed objective facts.

    O_i(t) is adjacent to P_i(t).  An objective-only lifecycle change therefore does
    not imply invalidation of PPP/Phi/H/G/R.  Objective-dependent discovery,
    Graph/seed visibility, Pi artifacts, responsiveness records, and downstream
    artifacts may be invalidated only when the host previously registered an
    explicit dependency on the changed objective identity/set/version.

    If the host also reports an actual/perceived-state change, this helper refuses to
    infer the state-driven invalidation.  That independent change must enter through
    the ordinary governance dependency/invalidation path.
    """
    from .models import (
        GovernanceDependencyChange, ObjectiveLifecycleInvalidationAssessment,
    )
    violations: list[str] = []
    if not change.change_id.strip() or not change.actor_id.strip() or not change.reason.strip():
        violations.append("change_id, actor_id, and reason must be nonempty")
    if not change.prior_objective_set_id.strip() or not change.current_objective_set_id.strip():
        violations.append("prior/current objective_set_id must be nonempty")
    if not change.prior_objective_set_version.strip() or not change.current_objective_set_version.strip():
        violations.append("prior/current objective_set_version must be nonempty")
    if not change.changed_objective_ids:
        violations.append("at least one changed objective identity is required")
    if len(change.changed_objective_ids) != len(set(change.changed_objective_ids)):
        violations.append("changed objective identities must be unique")
    if len(change.evidence_ids) != len(set(change.evidence_ids)):
        violations.append("evidence identities must be unique")
    if (change.prior_objective_set_id == change.current_objective_set_id
            and change.prior_objective_set_version == change.current_objective_set_version):
        violations.append("objective lifecycle change requires a changed set identity or version")
    if change.actual_state_changed or change.perceived_state_changed:
        violations.append(
            "objective lifecycle helper accepts objective-only changes; independent S/P changes require ordinary dependency invalidation"
        )
    objective_fact_ids = tuple(dict.fromkeys((
        f"objective_set:{change.prior_objective_set_id}:{change.prior_objective_set_version}",
        f"objective_set:{change.current_objective_set_id}:{change.current_objective_set_version}",
        *(f"objective:{oid}" for oid in change.changed_objective_ids),
    )))
    if violations:
        empty = GovernanceReentryAssessment(True, (), (), None, False, (), ("no objective invalidation derived",))
        return ObjectiveLifecycleInvalidationAssessment(
            False, change.change_id, objective_fact_ids, (), (), empty,
            preserved_stage_ids=("PPP", "Phi", "H", "G", "R"),
            violations=tuple(violations),
        )
    generic = GovernanceDependencyChange(
        change_id=change.change_id,
        fact_kind="objective",
        changed_fact_ids=objective_fact_ids,
        reason=change.reason,
        evidence_ids=change.evidence_ids,
        notes=("derived from explicit objective lifecycle change",),
    )
    derived = derive_dependency_invalidations(dependencies, (generic,))
    return ObjectiveLifecycleInvalidationAssessment(
        valid=derived.valid,
        change_id=change.change_id,
        objective_fact_ids=objective_fact_ids,
        matched_dependency_ids=derived.matched_dependency_ids,
        invalidation_signals=derived.invalidation_signals,
        reentry=derived.reentry,
        preserved_stage_ids=("PPP", "Phi", "H", "G", "R"),
        violations=derived.violations,
        notes=(
            "objective-only lifecycle change does not itself invalidate PPP/Phi/H/G/R",
            "only explicitly registered objective dependencies are invalidated",
            "no operator is recomputed and no objective-derived authority is created",
        ),
    )
