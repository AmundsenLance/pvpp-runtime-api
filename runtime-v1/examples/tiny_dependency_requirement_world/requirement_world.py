...
class RequirementWorld:
    """Four governing domains used to distinguish all-of from any-of prerequisites."""
    ...
    def project(self, state, action):
        ...
        if action.id == "steady":
            for d in vals:
                vals[d]-=10.0
        elif action.id == "preserve_b":
            vals.update(B=50.0, C=40.0, D=50.0)
        elif action.id == "preserve_c":
            vals.update(B=40.0, C=50.0, D=50.0)
        elif action.id == "lose_both":
            vals.update(B=40.0, C=40.0, D=60.0)
